...
- 本仓库 `src-tauri/Cargo.toml` 的默认 feature 是 `["desktop"]`，而上游 `v3.15.0` 仍是 `default = []`；后续维护 CI / 本地验证命令时不能再依赖隐式默认 feature，需显式区分 `desktop` 编译检查与 `headless,test-hooks` 后端测试，避免把上游同一句 `cargo test` 放大成 fork 专属的重型桌面测试。
